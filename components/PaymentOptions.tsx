...
export default function PaymentOptions({
  ...
  useEffect(() => {
    if (selected === 'apple_pay' && !paymentMethods.applePay) {
      setSelected('card')
    }
    if (selected === 'google_pay' && !paymentMethods.googlePay) {
      setSelected('card')
    }
  }, [selected, paymentMethods.applePay, paymentMethods.googlePay])
  ...
  const handleMethodChange = (method: PayMethod) => {
    setSelected(method)
  }
  ...
  return (
    <div className="space-y-4">
      <fieldset>
        <legend className="mb-3 text-sm font-semibold text-white">Payment method</legend>
          ...
          {paymentMethods.applePay && (
            <label
              className={`flex cursor-pointer items-center gap-3 rounded-lg border px-3 py-2.5 transition ${
                selected === 'apple_pay' ? 'border-emerald-500/70 bg-emerald-950/30' : 'border-zinc-600 hover:border-zinc-500'
              } ${probing ? 'pointer-events-none opacity-50' : ''}`}
            >
              <input
                type="radio"
                name="pay-method"
                className="h-4 w-4 text-emerald-500"
                checked={selected === 'apple_pay'}
                onChange={() => handleMethodChange('apple_pay')}
                disabled={probing}
              />
              <span className="text-sm text-zinc-200">Apple Pay</span>
            </label>
          )}
          {paymentMethods.googlePay && (
            <label
              className={`flex cursor-pointer items-center gap-3 rounded-lg border px-3 py-2.5 transition ${
                selected === 'google_pay' ? 'border-emerald-500/70 bg-emerald-950/30' : 'border-zinc-600 hover:border-zinc-500'
              } ${probing ? 'pointer-events-none opacity-50' : ''}`}
            >
              <input
                type="radio"
                name="pay-method"
                className="h-4 w-4 text-emerald-500"
                checked={selected === 'google_pay'}
                onChange={() => handleMethodChange('google_pay')}
                disabled={probing}
              />
              <span className="text-sm text-zinc-200">Google Pay</span>
            </label>
          )}
          <label
            className={`flex cursor-pointer items-center gap-3 rounded-lg border px-3 py-2.5 transition ${
              selected === 'card' ? 'border-emerald-500/70 bg-emerald-950/30' : 'border-zinc-600 hover:border-zinc-500'
            }`}
          >
            <input
              type="radio"
            ...
            />
            <span className="text-sm text-zinc-200">Credit or debit card</span>
          </label>
        </div>
        {!probing && !canMakePayment && <p className="mt-2 text-xs text-zinc-500">{expressHint}</p>}
      </fieldset>
      ...
      {selected === 'apple_pay' && canMakePayment && paymentMethods.applePay && (
        <div className="pt-1">
          <button
            type="button"
            onClick={showExpress}
      ...
      {selected === 'google_pay' && canMakePayment && paymentMethods.googlePay && (
        <div className="pt-1">
          <button
            type="button"
            onClick={showExpress}
